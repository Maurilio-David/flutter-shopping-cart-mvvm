...
import 'service/cart_api_test.dart';

class FakeCartViewModel extends ChangeNotifier {
  final FakeCartApi _api = FakeCartApi();
...
void main() {
  test('remove item failure does not remove product', () async {
    final vm = FakeCartViewModel();

    vm.add(fakeProduct);
    expect(vm.items.length, 1);

    await vm.remove(fakeProduct);

    expect(vm.removeCommand.hasError, true);
    expect(vm.items.length, 1);
  });
}
